...
class fvCommunityNewsCaptcha {
	...
	public function render() {
		$width = round(($this->_fontSize+($this->_fontSize*.3)) * $this->_textLength);
		$height = round($this->_fontSize*2);
		...
		for ($i = 0; $i < $this->_textLength; $i++) {
			...
			$y = round( mt_rand((4/6)*$height, (5/6)*$height) );
			...
			$x_left += ($this->_fontSize+4);
		}
		
		$x_end = $width;
		$number = mt_rand(round($this->_fontSize*.1), round($this->_fontSize*.4));
		$spread = 0;
		/*
		for ($i = 0; $i<$number; $i++) {
