...
class fvCommunityNewsBootstrap extends fvCommunityNewsBootstrap_Abstract {
	...
	protected function _initInstall() {
		if (!get_option($this->_settings->getPrefix() . 'Version')) {
			$install = new fvCommunityNewsInstall($this->_settings);
			$install->installDatabase()
					->installSettings();
		} elseif (version_compare($this->_settings->get('Version'), $this->_settings->getDefault('Version'), '<')) {
			$update = new fvCommunityNewsUpdate($this->_settings);
	...
	protected function _initView() {
		$view = new fvCommunityNewsView(FVCN_ROOTDIR);
		fvCommunityNewsView::setInstance($view);
		fvCommunityNewsRegistry::set('view', $view);
	}
	...
	protected function _initAjaxRequest() {
		if (isset($_REQUEST['fvcn-ajax-request'])) {
			new fvCommunityNewsAjaxRequest();
			exit;
		}
	}
