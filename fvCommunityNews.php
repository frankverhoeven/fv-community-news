...
function fvCommunityNewsInit() {
	...
	try {
		...
		if (WP_DEBUG) {
			wp_die(
				'<h1>Fv Community News Error</h1>' .
				'<h2>Caught Exception</h2><p style="margin-top: 0;"> ' . $e->getMessage() . '</p>' .
				'<h2>File</h2><p style="margin-top: 0;">' . $e->getFile() . '</p>' .
				'<h2>Line</h2><p style="margin-top: 0;">' . $e->getLine() . '</p>' .
				'<h2>Stack Trace</h2><pre style="overflow: auto;">' . $e->getTraceAsString() . '</pre></p>'
			);
		}
	}
}
