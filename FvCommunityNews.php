...
if (!class_exists('FvCommunityNews')) {
	...
	final class FvCommunityNews {
		...
		private function _loadFiles() {
			...
			$loader = new FvCommunityNews_Loader();
		...
		private function _setupActions() {
			...
			FvCommunityNews_Request::setInstance( new FvCommunityNews_Request() );
			
			
			$fvcnRss = new FvCommunityNews_Rss();
			FvCommunityNews_Registry::getInstance()->rss = $fvcnRss;
			add_action('init', array($fvcnRss, 'addFeed'));
		}
