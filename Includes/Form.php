...
abstract class FvCommunityNews_Form {
	 ...
	 *		@return object $this
	 */
	public function setGroupSuffix($suffix) {
		$this->_groupSuffix = $suffix;
		return $this;
	}
	...
	public function getAllGroupElements() {
		...
		foreach ($this->getGroups() as $name=>$elements) {
	...
	public function renderGroups() {
		...
		foreach ($this->getGroups() as $name=>$elements) {
			$group = $this->getGroupPrefix() . $this->renderElements($elements) . $this->getGroupSuffix();
			
			$group = str_replace('%name%', $name, $group);
