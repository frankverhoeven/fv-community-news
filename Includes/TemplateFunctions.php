...
function fvcn_get_form() {
	if (fvcn_get_setting('LoggedInToPost') && !is_user_logged_in()) {
		...
		return apply_filters('fvcn_form', FvCommunityNews_Request::getForm('add-post')->render());
	}
}
...
function fvcn_get_form_message() {
	return apply_filters('fvcn_form_message', FvCommunityNews_Request::getForm('add-post')->getMessage());
}
...
function fvcn_form_processed() {
	return FvCommunityNews_Request::getForm('add-post')->isProcessed();
}
