...
class FvCommunityNews_Akismet
	...
	private function sendRequest($request, $host, $path) {
		...
		$socketWriteRead = new FvCommunityNews_SocketWriteRead($host, $this->apiPort, $http_request);
	 ...
	 *		@return object $this
	 */
	public function setCommunityNews(FvCommunityNews_Models_Post $post) {
		$this->setCommentType('community_news')
			 ->setCommentAuthor($post->Author)
			 ->setCommentAuthorEmail($post->AuthorEmail)
			 ->setCommentAuthorURL($post->Url)
 ...
 *
 *	@package	akismet
 *	@name		FvCommunityNews_SocketWriteRead
 *	@version	0.1
 *  @author		Alex Potsides
 *  @link		http://www.achingbrain.net/
 */
class FvCommunityNews_SocketWriteRead {
	private $host;
	private $port;
	private $request;
	private $response;
